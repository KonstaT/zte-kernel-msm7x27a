...
//ZTE_JHT_20120424+++
	...
	if(net_os_get_suspend(dev) == 1 && (pm == 2)) {
		pm = 1;
		err = wldev_ioctl(dev, WLC_SET_PM, &pm, sizeof(pm), true);
	} else 
		err = wldev_ioctl(dev, WLC_SET_PM, &pm, sizeof(pm), true);
	...
	struct beacon_parameters *info)
{
	...
	if (p2p_is_on(wl) &&
		...
		if ((ssid_ie = bcm_parse_tlvs((u8 *)&info->head[ie_offset],
			info->head_len - ie_offset,
			DOT11_MNG_SSID_ID)) != NULL) {
			//ZTE_JHT_20120312 for hidden ssid+++
			int closed = 0;
//Hector 20120618 add for open softap max client bug+++
			int max_assoc = 8;
...
//ZTE_JHT_20120516 for channel error+++
	//static int iii;
...
#else
//ZTE_JHT_20120516 for channel error+++
	//notif_bss_info->channel = iii++;
	printk("SSID:\"%s\" \n", bi->SSID);
	printk("notif_bss_info=%p \n", notif_bss_info);
	printk("notif_bss_info->channel=%d \n", notif_bss_info->channel);
	printk("band=%p \n", band);
	printk("band->band=%d \n", band->band);
	
	//notif_bss_info->channel = iii++;
	//freq = ieee80211_channel_to_frequency(notif_bss_info->channel, band->band);
	freq = ieee80211_channel_to_frequency(notif_bss_info->channel, 0);//we only use 2ghz
...
//ZTE_JHT_20120516 for channel error+++
	if((notif_bss_info->channel > 14) && (notif_bss_info->channel < 20)) {
		printk("test channel error, channel = %d \n", notif_bss_info->channel);
		return -ENOMEM;
	}
//ZTE_JHT_20120516 for channel error---
