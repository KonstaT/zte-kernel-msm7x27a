...
static void dhd_suspend_resume_helper(struct dhd_info *dhd, int val)
{
	...
	dhdp->in_suspend = val;
	DHD_ERROR(("%s: dhdp->suspend_disable_flag=%d\n", __FUNCTION__, dhdp->suspend_disable_flag));
...
static void dhd_early_suspend(struct early_suspend *h)
{
	...
	if (dhd&&dhd->dhd_state&DHD_ATTACH_STATE_FW_DONE)//zte_wifi_zhp_20110902 add  //shaohua add for NULL point when suspend and load wifi driver
		dhd_suspend_resume_helper(dhd, 1);
	else
		printk("%s: dhd is not attach,skip it! dhd_state: 0x%.08x\n", __FUNCTION__, dhd->dhd_state);//zte_wifi_zhp_20110902 add 
...
static void dhd_late_resume(struct early_suspend *h)
{
	...
	printk("%s: dhd is not attach,skip it!\n", __FUNCTION__);//zte_wifi_zhp_20110902 add
...
error_exit:
        /* at least you need turn off the power if error occurs after turn on the wifi power  */
#if defined(WL_CFG80211) && defined(OEM_ANDROID)
	if (ifidx == 0 && !dhd_download_fw_on_driverload)
        {
            printk("%s, UP failed, so shutdown the power\n", __FUNCTION__);
	    wl_android_wifi_off(net);
       }
#endif /* defined(WL_CFG80211) && defined(OEM_ANDROID) */
        DHD_OS_WAKE_UNLOCK(&dhd->pub);
...
dhd_bus_start(dhd_pub_t *dhdp)
{
	...
	dhd->dhd_state |= DHD_ATTACH_STATE_FW_DONE;   //shaohua add for NULL point when suspend and load wifi driver
	DHD_ERROR(("%s: dhd_state=0x%.08x\n", __FUNCTION__, dhd->dhd_state));
...
//ZTE_JHT_20120531 for white list+++
#if 1	//shaohua add for white list
...
//ZTE_JHT_20120326---
	...
	if (ap_fw_loaded == FALSE)
		...
		printk("shaohua check keep_alive \n");
