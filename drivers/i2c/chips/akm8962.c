...
static int AKECS_GetData(char *rbuf, int size)
{
	...
	return 0;
#endif
	if(size < SENSOR_DATA_SIZE + 1)
	  return -EINVAL;

	// read C0 - C4
	rbuf[0] = AK8962_FUSE_ASAX;
	return AKI2C_RxData(rbuf, SENSOR_DATA_SIZE+1);
}
...
int akm8962_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (err < 0) {
		printk(KERN_ERR 
				"%s:set power down mode error or akm8962 does not exsit,AKECS_CheckDevice() return %d\n",__FUNCTION__,err);
		remove_compass = 1;
	}
	#if 0
	/* IRQ */
...
static int akm8962_remove(struct i2c_client *client)
{
	...
	remove_compass = -1; //FYA_ADD_20110308 +++++++++
...
static int __init akm8962_init(void)
{
	...
	ret = misc_register(&akmd_device);
	if (ret) {
		printk(KERN_ERR
			   "%s: akmd_device register failed\n",__FUNCTION__);
	}else {
		printk(KERN_INFO
			   "%s: akmd_device register success\n",__FUNCTION__);
	}
	
	ret = misc_register(&akm_aot_device);
	if (ret) {
		printk(KERN_ERR
		       "%s: akm_aot_device register failed\n",__FUNCTION__);
	}else {
		printk(KERN_INFO
		       "%s: akm_aot_device register success\n",__FUNCTION__);
	}
