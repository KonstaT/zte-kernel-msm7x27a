...
static int lis302dl_hw_init(void)//Qchen 20111012
{
	char buf[4];
	int ret;
	//power up
	   ...
	   unsigned long arg)
{
	...
	case LIS302DL_IOCTL_INIT:
		ret = lis302dl_hw_init();//QChen 20111012  cancel variable : lis302dl_pdata
		if (ret < 0)
			return ret;
		break;
	case LIS302DL_IOCTL_READ:
...
/* QChen commend in 20111012
	*/   
	err = lis302dl_hw_init();//lis302dl_hw_init(lis302dl_pdata);
	if (err < 0) {
