...
#define ENABLE_QVGA_HIMAX   1 //pan add 
...
#if ENABLE_QVGA_HIMAX

typedef enum
{
	LCD_PANEL_NONE = 0,
	LCD_PANEL_QVGA_HIMAX,
	LCD_PANEL_QVGA_LEAD	
}LCD_PANEL_TYPE;
...
/*	gpio_lcd_emuspi_write_one_para(0x22,0x00); */
	...
	printk("lcd module himax init exit\n!");
...
static void lcd_panel_sleep(void)
{
	switch(g_lcd_panel_type)
		{
			case LCD_PANEL_QVGA_HIMAX:
				lcdc_himax_sleep();
				mdelay(10);
				break;
			case LCD_PANEL_QVGA_LEAD:
				lcdc_lead_sleep();
				mdelay(10);
				break;
			default:
...
*/
	...
	printk("lcd module lead init exit\n!");
...
static LCD_PANEL_TYPE lcd_panel_detect(void)
{
	...
	gpio_lcd_emuspi_read_one_para(0x00,&panel_id);
	printk("lcd_panel_detect is 0x%x\n!",panel_id);
	if ( panel_id == 0x47)	
	{
		return	LCD_PANEL_QVGA_HIMAX;
	}

    else 
    {	

	   return  LCD_PANEL_QVGA_LEAD;
	   /* gpio_lcd_emuspi_read_one_para1(0x00,&panel_id);
...
static int __devinit lcdc_panel_probe(struct platform_device *pdev)
{
	...
	if(pdev->id == 0) {
		...
		if (g_lcd_panel_type == LCD_PANEL_QVGA_HIMAX)
		{
		...
		else if (g_lcd_panel_type == LCD_PANEL_QVGA_LEAD)
		{
