...
#define GPIO_LCD_BL_SC_OUT 88
#define GPIO_LCD_BL_EN

#define GPIO_LCD_ID			48 //zte_gequn091966,20110705

#define GPIO_SPI_CS          122
#define GPIO_SPI_SDO         123
#define GPIO_SPI_SCLK        124
#define GPIO_SPI_SDI         109
...
static void lcdc_set_bl(struct msm_fb_data_type *mfd)
{
	...
	if(current_lel<3)
	{
    	...
    	pmic_gpio_direction_output(PMIC_GPIO_1);
    	//pmic_gpio_direction_output(1);
		pmic_gpio_set_value(PMIC_GPIO_1, 0);
		//pmic_gpio_set_value(1, 0);
	}
	else
		{
    	...
    	pmic_gpio_direction_output(PMIC_GPIO_1);
    	//pmic_gpio_direction_output(1);
		pmic_gpio_set_value(PMIC_GPIO_1, 1);
		//pmic_gpio_set_value(1, 1);
	}
	//return;
...
printk("lcd module hydis gequn  init finish\n!");
   #if 1
       mdelay(10);
  ...
  SPI_Start();
	...
	gpio_lcd_truly_emuspi_write_one_data(0x91);
...
void lcdc_trulylg_init(void)
{
		...
		SPI_Start();
		gpio_lcd_truly_emuspi_write_one_index(0xb4);  // set display  480x800 
        gpio_lcd_truly_emuspi_write_one_data(0x00);           //0x01
        gpio_lcd_truly_emuspi_write_one_data(0x0c);   
        gpio_lcd_truly_emuspi_write_one_data(0xa0); 
        gpio_lcd_truly_emuspi_write_one_data(0x0e);   
        gpio_lcd_truly_emuspi_write_one_data(0x06);  
...
static void spi_init(void)
{
   struct pm8xxx_gpio_rpc_cfg param=
...
static int __devinit lcdc_panel_probe(struct platform_device *pdev)
{
	...
	if(pdev->id == 0) {     
		...
		{
			case LCD_PANEL_HYDIS_WVGA:
				LcdPanleID=(u32)101;   //ZTE_LCD_LHT_20100611_001
			...
			case LCD_PANEL_LEADCPT_WVGA:
				...
				break;
			case LCD_PANEL_TRULYLG_WVGA:
				LcdPanleID=(u32)100;   //ZTE_LCD_LHT_20100611_001
				pinfo->clk_rate = 24576000;
				ret = platform_device_register(&this_device);
				break;
			case LCD_PANEL_OLED_WVGA:
