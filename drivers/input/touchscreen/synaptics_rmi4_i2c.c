...
#include <linux/input/synaptics_rmi4_i2c.h>
...
#if defined(CONFIG_TOUCHSCREEN_SYNAPTICS_RMI4_I2C_FW)
int syna_update_result_flag=0;
extern int syna_fwupdate(struct i2c_client *client);
...
#if defined (CONFIG_MACH_ATLAS40)//P740A  800*480   1731*1040    MENU HOME BACK SEARCH 
static const char ts_keys_size[] = "0x01:139:40:850:60:60:0x01:102:170:850:60:60:0x01:158:300:850:60:60:0x01:217:430:850:60:60";
#else
static const char ts_keys_size[] = "0x01:139:50:840:60:60:0x01:102:180:840:60:60:0x01:158:310:840:60:60:0x01:217:440:840:60:60";
#endif 
			  ...
			  void *data)
{
	...
	ret = i2c_smbus_read_i2c_block_data(
		ts->client, ts->f34.ctrl_base, 4, synaptics_config_id);
	if (ret < 0){
		...
		switch (synaptics_config_id[0]){//chip type
		case '1':
			len += sprintf(page + len, "chip type : S2200 \n" );
			break;
		case '2':
			len += sprintf(page + len, "chip type : S2202 \n" );
			break;
		case '3':
			len += sprintf(page + len, "chip type : S3200 \n" );
			break;
		case '4':
			len += sprintf(page + len, "chip type : S3202 \n" );
			break;
		case '5':
			len += sprintf(page + len, "chip type : S3203 \n" );
			break;
		case '6':
			len += sprintf(page + len, "chip type : S7020 \n" );
			break;
		case '7':
			len += sprintf(page + len, "chip type : S7300 \n" );
			break;
		default:
			len += sprintf(page + len, "chip type : unknown \n");
		}

		switch (synaptics_config_id[1]){//sensor partner
		case '1':
			len += sprintf(page + len, "sensor partner : TPK \n" );
			break;
		case '2':
			len += sprintf(page + len, "sensor partner : Truly \n" );
			break;
		case '3':
			len += sprintf(page + len, "sensor partner : Success \n" );
			break;
		case '4':
			len += sprintf(page + len, "sensor partner : Ofilm \n" );
			break;
		case '5':
			len += sprintf(page + len, "sensor partner : Lead \n" );
			break;
		case '6':
			len += sprintf(page + len, "sensor partner : Wintek \n" );
			break;
		case '7':
			len += sprintf(page + len, "sensor partner : Laibao \n" );
			break;
		case '8':
			len += sprintf(page + len, "sensor partner : CMI \n" );
			break;
		case '9':
			len += sprintf(page + len, "sensor partner : ECW \n" );
			break;
		case 'A':
			len += sprintf(page + len, "sensor partner : Goworld \n" );
			break;
		default:
			len += sprintf(page + len, "sensor partner : unknown \n" );
		}

		len += sprintf(page + len, "FW Revision : %c%c\n", synaptics_config_id[2], synaptics_config_id[3]);
#ifdef CONFIG_TOUCHSCREEN_SYNAPTICS_RMI4_I2C_FW
		len += sprintf(page + len, "Update flag : 0x%x\n", syna_update_result_flag);
#endif
...
#if defined(CONFIG_TOUCHSCREEN_SYNAPTICS_RMI4_I2C_FW)
	int ret=0;
	syna_update_result_flag=0;

	disable_irq(pgtsdata->client->irq);

	ret = syna_fwupdate(pgtsdata->client);
	if  (ret < 0){
		printk("%s: fts fw update failed!\n", __func__);
		syna_update_result_flag=1;
		return -EINVAL;
	}
		...
		if (ret < 0){
			...
			*/
			...
			case 0x11:
				...
				pr_info("%s: ts->max[0]=%d, ts->max[1]=%d\n", __func__, ts->max[0], ts->max[1]);
...
/*			case 0x05:
	...
	ret = i2c_smbus_read_i2c_block_data(ts->client, ts->f34.ctrl_base, 4, synaptics_config_id);
	if (ret < 0){
		pr_err("%s: failed to get ts f34.ctrl_base\n",__func__);
		return ret;
	}
	pr_info("%s: Config ID : %c%c%c%c\n",__func__, \
		synaptics_config_id[0], synaptics_config_id[1], synaptics_config_id[2], synaptics_config_id[3]);
	
	switch (synaptics_config_id[0]){//chip type
	case '1':
		pr_info("%s: chip type : S2200 \n", __func__ );
		break;
	case '2':
		pr_info("%s: chip type : S2202 \n", __func__);
		break;
	case '3':
		pr_info("%s: chip type : S3200 \n", __func__);
		break;
	case '4':
		pr_info("%s: chip type : S3202 \n", __func__);
		break;
	case '5':
		pr_info("%s: chip type : S3203 \n", __func__);
		break;
	case '6':
		pr_info("%s: chip type : S7020 \n", __func__);
		break;
	case '7':
		pr_info("%s: chip type : S7300 \n", __func__);
		break;
	default:
		pr_info("%s: chip type : unknown \n", __func__);
	}
	
	switch (synaptics_config_id[1]){//sensor partner
	case '1':
		pr_info("%s: sensor partner : TPK \n", __func__);
		break;
	case '2':
		pr_info("%s: sensor partner : Truly \n", __func__);
		break;
	case '3':
		pr_info("%s: sensor partner : Success \n", __func__);
		break;
	case '4':
		pr_info("%s: sensor partner : Ofilm \n", __func__);
		break;
	case '5':
		pr_info("%s: sensor partner : Lead \n", __func__);
		break;
	case '6':
		pr_info("%s: sensor partner : Wintek \n", __func__);
		break;
	case '7':
		pr_info("%s: sensor partner : Laibao \n", __func__);
		break;
	case '8':
		pr_info("%s: sensor partner : CMI \n", __func__);
		break;
	case '9':
		pr_info("%s: sensor partner : ECW \n", __func__);
		break;
	case 'A':
		pr_info("%s: sensor partner : Goworld \n", __func__);
		break;
	default:
		pr_info("%s: sensor partner : unknown \n", __func__);
	}
	
	pr_info("%s: FW Revision : %c%c\n", __func__, synaptics_config_id[2], synaptics_config_id[3]);
	}
...
*/
	...
	if ( ts->f11.flag && (interrupt & ts->f11.intr_mask) ) 
	{
		...
		if (will_report) {
			...
			if(new_report_data[0].y >=820)
			{
				//input_report_key(ts->input_dev, BTN_9, !!new_report_data[0].z);// recovery mode use
			}
#endif			
...
/*
	ret = gpio_direction_input(ts->gpio_irq);
	if (ret) {
		pr_err("%s: unable to set_direction for synaptics irq gpio [%d]\n",
				__func__, ts->gpio_irq);
		goto err_power_failed;
	}
*/
	...
	max_y = 1747-6;//1731*86.20/94.50=1579
#else
	max_y=ts->max[1];
#endif
...
#ifdef TOUCHSCREEN_DUPLICATED_FILTER
{
	...
	if (client->irq) 
	{
		ts->use_irq = 1;//ZTE_TS_XYM_20110923
		...
		{
			ts->use_irq = 0;//ZTE_TS_XYM_20110923
