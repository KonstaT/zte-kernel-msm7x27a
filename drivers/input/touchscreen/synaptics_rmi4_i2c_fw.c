...
extern char synaptics_config_id[];

static struct i2c_client *syna_i2c_client;

#if defined (CONFIG_MACH_ATLAS40)
#define CTPM_FW_NAME "PR1101200-s2202_zte_30303039-ATLAS40-truly.img"
#endif
...
static void synaptics_read_firmware_header( struct i2c_client *client,u8 *pfwfile, u32 filesize)
{
	...
	//add by xym    根据IC type决定是否需要比较sensor partner ID
	pr_info("%s: chip type - read char %c(0x%x) from device. \n", __func__, synaptics_config_id[0], synaptics_config_id[0]);
	if((synaptics_config_id[0]!=0x0)//not 3k series IC 
		&&(synaptics_config_id[0]!='0') )//not ATLAS40 0039 img
	{
		//compare sensor partner id
		pr_info("%s: sensor partner - read char %c(0x%x) from img, read char %c(0x%x) from device. \n", \
			__func__, *(config_imgdata+1), *(config_imgdata+1), synaptics_config_id[1], synaptics_config_id[1]);
		if(*(config_imgdata+1) != synaptics_config_id[1])
		{
			pr_err("%s: sensor partner not match!!!\n", __func__);
			while(1);
		}
	}
	//end by xym
...
static void syna_power_on_off(struct i2c_client *client,int on_off)
{
	...
	if (ts && ts->power) {
		ret = ts->power(on_off);
		if (ret < 0)
			pr_err("syna_power_on_off failed\n");
		else
			printk("syna_power_on_off %d \n",on_off);
	}
						...
						const char *buf, size_t count)
{
   ///	int fwsize;
	...
	fwname[count-1] = '\0';
	printk("xiayc: %s, fwname=%s\n", __func__, fwname);

	//syna_fw_update( client, fwname, fwsize);
	syna_fw_update( client, fwname );
...
int syna_fwupdate(struct i2c_client *client) 
{
	//struct i2c_client *client = syna_i2c_client;
	//uint fwsize;
...
*/
	...
	syna_fw_update( client, (u8 *)CTPM_FW_NAME);
