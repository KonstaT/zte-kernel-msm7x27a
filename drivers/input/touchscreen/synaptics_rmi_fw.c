...
static int synaptics_enable_flash(struct i2c_client *client)
{
	...
	ret = synaptics_get_pgt_f34(client);
	if(ret<0) return -1;
...
1, match
*/
static int synaptics_check_config_id(struct i2c_client *client)
{
	...
	pr_info("current chip type id = 0x%x(%c)\n",
		ts->config_id.chip_type, ts->config_id.chip_type);
 	 if ( ( ts->config_id.chip_type == 0x0 )	// 3k series IC 
	 	|| ( ts->config_id.chip_type == 0x30) )	// ASCII '0': ATLAS40 0039 img
		return 1;

	
	// check sensor partner id
	pr_info("current sensor partner id = 0x%x(%c)\n",
		ts->config_id.sensor,ts->config_id.sensor);
	pr_info("fw file sensor partner id = 0x%x(%c)\n",
		*(config_imgdata+1),*(config_imgdata+1));

	// id mached?
	if( *(config_imgdata+1) != ts->config_id.sensor)
		return 0;
	else
		return 1;
	...
	char *pfwfilename /*,u32 fwsize*/)
{
	...
	//RMI4Init();
	ret = syna_i2c_init(client);
	if(ret<0) return -1;
	
	ret = syna_rmi4_init(client);
	if(ret<0) return -1;
	
	synaptics_set_flash_addr(client);
	
	//前面准备工作完毕，开始programming流程
	ret = synaptics_enable_flash(client);
	if(ret<0) return -1;
	...
	if ( !synaptics_check_config_id(client) ){
		pr_err("sensor partner id dismatch!\n");
		ret = -1;
		goto malloc_error;
	}
	ret = synaptics_prog_firmware(client);
	if(ret<0)
	{
		ret = -1;
		goto malloc_error;
	}	
	ret = synaptics_prog_configuration(client);
	if(ret<0)
	{
		ret = -1;
		goto malloc_error;
	...
	ret = synaptics_disenable_flash(client);
	if(ret<0)
	{
		ret = -1;
		goto malloc_error;
