...
struct pn544_dev    *pn544_dev = NULL; //added by zhang.yu_1 for nfc restart before shut down @120218
...
static long pn544_dev_ioctl(struct file *filp,unsigned int cmd, unsigned long arg)
{
	...
	case PN544_SET_PWR:
		if (arg == 2) {
			...
			printk("%s power on with firmware,irq=%d\n", __func__,client->irq);
			...
			printk("%s power on with ioctl cmd,irq=%d\n", __func__,client->irq);
			...
			printk("%s power off with ioctl cmd,irq=%d\n", __func__,client->irq);
...
//Begin ZTE_XJB_PM_20120427
//added by zhang.yu_1 for nfc restart before shut down @120218
static void pn544_shutdown(struct i2c_client *client)
{
   printk(KERN_INFO "pn544_shutdown()\n");
